...
class adminModel extends BaseModel
{
    public function get_all_clients()
    {
        ...
        $results = $this->query("SELECT p.id, AES_DECRYPT(p.name, '" . MYSQL_AES_KEY . "') name, " .
            "p.gender, p.birthdate, AES_DECRYPT(p.email, '" . MYSQL_AES_KEY . "') email, " .
            "AES_DECRYPT(p.phone, '" . MYSQL_AES_KEY . "') phone, p.interests, p.created_at, " .
            "AES_DECRYPT(a.name, '" . MYSQL_AES_KEY . "') agent FROM persons p LEFT JOIN agents a " .
            "ON p.id_agent = a.id WHERE p.deleted_at IS NULL ORDER BY created_at DESC");
    ...
    // Funções de sql para pegar dados especificos

    public function get_agents_for_management()
    {
        $this->db_connect();
        $results = $this->query("SELECT id, AES_DECRYPT(name,'" . MYSQL_AES_KEY . "')name, passwrd, profile, last_login, created_at, updated_at, deleted_at FROM agents");
    ...
    public function check_name($name)
    {
        ...
        $results = $this->query("SELECT id FROM agents WHERE AES_ENCRYPT(:name, '" . MYSQL_AES_KEY .  "') = name", $params);
    ...
    public function add_agent($agent_data)
    {
        // purl é um código criado para cada agente que for criado, e será enviado um email para o próprio enviando esse purl como código
        $chars = 'abcdefghijkabcdefghijkabcdefghijkABCDEFGHIJKABCDEFGHIJKABCDEFGHIJK';
        $purl = substr(str_shuffle($chars), 0, 20);
        
        $params = [
            ':name' => $agent_data['text_name'],
            ':profile' => $agent_data['select_profile'],
            ':purl' => $purl
        ];
    ...
    public function get_agent_data($id)
    {
        ...
        $results = $this->query("SELECT id, AES_DECRYPT(name, '" . MYSQL_AES_KEY . "') name, profile FROM agents WHERE id = :id", $params);
    ...
    public function edit_agent($id)
    {
        ...
        $results = $this->non_query('UPDATE agents SET name = AES_ENCRYPT(:name, "' . MYSQL_AES_KEY . '"), profile = :profile, updated_at = NOW() WHERE id = :id', $params);
    ...
    public function delete_agent($id)
    {
        ...
        $this->db_connect();
        $this->non_query('UPDATE agents SET deleted_at = NOW(), updated_at = NOW() WHERE id = :id', $params);
    }
    ...
    public function recover($id)
    {
        ...
        $this->db_connect();
        $this->non_query('UPDATE agents SET deleted_at = NULL, updated_at = NOW() WHERE id = :id', $params);
    }
